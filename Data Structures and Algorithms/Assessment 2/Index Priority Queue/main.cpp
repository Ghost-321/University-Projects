...
// Checks in descending order and are popped in ascending
TEST(IndexPriorityQueueTest, pushDescendingOrder) {
  IndexPriorityQueue<int> heap(4);
  heap.push(4, 0);
  heap.push(3, 1);
  heap.push(2, 2);
...
//inserting and altering key to higher priority e.g. ensure heap is properly maintained
TEST(IndexPriorityQueueTest, changeKeyIncreasePriority) {
  IndexPriorityQueue<int> heap(4);
  heap.push(1, 0);
  heap.push(2, 1);
  heap.push(3, 2);
...
//Tests inserting key as a lower priority e.g. ensure heap is properly maintained
TEST(IndexPriorityQueueTest, changeKeyDecreasePriority) {
  IndexPriorityQueue<int> heap(4);
  heap.push(4, 0);
  heap.push(3, 1);
  heap.push(2, 2);
...
//Tests that head can handle large sizes
TEST(IndexPriorityQueueTest, pushLargeNumberOfElements) {
  const int N = 1000;
  IndexPriorityQueue<int> heap(N);
  for (int i = 0; i < N; ++i) {
    heap.push(i, i);
  ...
  for (int i = 0; i < N; ++i) {
    ASSERT_EQ(heap.top().first, i);
    heap.pop();
  }
  ASSERT_TRUE(heap.empty());
}

//testing behavior when priority gets altered and element is removed
TEST(IndexPriorityQueueTest, changeKeyAndErase) {
  IndexPriorityQueue<int> heap(5);
  heap.push(5, 0);
  heap.push(3, 1);
  heap.push(4, 2);
  heap.changeKey(2, 0);
...
//test heaps reusability
TEST(IndexPriorityQueueTest, popAllAndReuse) {
  IndexPriorityQueue<int> heap(4);
  heap.push(4, 0);
  heap.push(3, 1);
  heap.push(2, 2);
