...
typename MyVector<T>::Iterator MyVector<T>::begin() const {
  return Iterator {arrayPointer_};
}

template <typename T>
typename MyVector<T>::Iterator MyVector<T>::end() const {
  return Iterator {arrayPointer_ + size_};
}

// member functions of MyVector<T>::Iterator
template <typename T>
MyVector<T>::Iterator::Iterator(T* input) : ptr {input} {}
