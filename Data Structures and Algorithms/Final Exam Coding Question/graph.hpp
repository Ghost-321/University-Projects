...
std::vector<int> longestLegDistance(const Graph <int>& G, int source) {
  int n = G.size();
  std::vector<int> distTo(n, std::numeric_limits<int>::max());
  distTo[source] = std::numeric_limits<int>::min();

  using Pair = std::pair<int, int>;
  std::priority_queue<Pair, std::vector<Pair>, std::greater<Pair>> pq;
  pq.push({distTo[source], source});
  ...
  while (!pq.empty()) {
    ...
    for (const auto& neighbour : *(G.neighbours(u))) {
      int v = neighbour.first;
      int weight = neighbour.second;
      int longestLeg = std::max(currentDist, weight);

      if (longestLeg < distTo[v]) {
        distTo[v] = longestLeg;
